...
export default function FichasTecnicasPage() {
  ...
  // Modal de produto de venda
  const [modal, setModal] = useState(false)
  const [editando, setEditando] = useState(null)
  const [form, setForm] = useState({ nome: '', descricao: '', categoria_id: '', tipo: 'revenda', unidade: 'un', estoque_minimo: '', valor_venda: '', dias_validade: '', unidade_insumo: '', fator_conversao: '' })
  ...
  function abrirNovo() {
    setEditando(null)
    setForm({ nome: '', descricao: '', categoria_id: '', tipo: 'revenda', unidade: 'un', estoque_minimo: '', valor_venda: '', dias_validade: '', unidade_insumo: '', fator_conversao: '' })
    setErroProduto('')
    setModal(true)
  }
  ...
  async function salvarProduto() {
    ...
    try {
      ...
      if (editando) {
        ...
        await api.post('/produtos', { ...payload, sku })
      }
      setModal(false)
      carregar()
  ...
  return (
    <AppLayout title="Fichas Tecnicas">
      <div className="flex items-center justify-between mb-4">
        ...
        {aba === 'produtos' && admin && (
          <button className="btn btn-primary" onClick={abrirNovo}>+ Novo produto de venda</button>
        )}
      </div>
          ...
          <div className="card" style={{ padding: '1rem' }}>
            ...
            {loading ? (
              ...
              <p className="text-sm text-muted" style={{ padding: '.5rem' }}>
                Nenhum produto de venda. Cadastre na aba "Produtos de Venda".
              </p>
            ) : (
                          ...
                          <tbody>
                            {ficha.map(item => (
                              <tr key={item.insumo_id}>
                                ...
                                <td>
                                  <button className="btn btn-danger btn-sm" onClick={() => removerItemLocal(item.insumo_id)}>Remover</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                          ...
                          >
                            ...
                            {insumos.filter(i => !ficha.find(f => f.insumo_id === i.id)).map(i => (
                              <option key={i.id} value={i.id}>
                                {i.nome} ({i.unidade_insumo || i.unidade})
                              </option>
                            ))}
                          </select>
                        </div>
        ...
        <div className="overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal">
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={salvarProduto} disabled={salvandoProduto || !form.nome || !form.categoria_id}>
